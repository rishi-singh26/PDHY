    ...
    try {
      auth.currentUser
        ? firestore
            ...
            .onSnapshot(
              (applications) => {
                let pharmacyApplications = [];
                applications.forEach((application) => {
                  const data = application.data();
                  const _id = application.id;
                  pharmacyApplications.push({ _id, ...data });
                });
                setIsLoading(false);
                setApplications(pharmacyApplications);
              },
              (err) => {
                setIsLoading(false);
                dispatch(
              ...
              <View style={styles.applicationContainer}>
                <View style={{ maxWidth: "75%" }}>
                  <View style={styles.horizontalView}>
                    ...
                    <Text style={styles.pharmacyEstablishedain}>
                      {item.establishedIn}
                    </Text>
                  </View>
